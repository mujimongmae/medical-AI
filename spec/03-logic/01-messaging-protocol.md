...
|------|-----------|---------|------|
| `HELLO` | 환자·이웃 | `{id}` | ws 연결 직후 신원 등록 → 서버가 소켓↔사람 매핑. **재접속 복원:** 이웃이면 서버가 진행 중(`notifying_neighbors`) 최신 호출 1건을 `NEIGHBOR_ALERT`로 재전송 → 푸시 탭으로 앱을 열었을 때(콜드스타트) '대기 중'이 아니라 **호출 화면**으로 진입 |
